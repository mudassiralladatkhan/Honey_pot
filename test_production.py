...
BASE_URL = "https://honeypot-production-db70.up.railway.app"
API_KEY = "test_key_123"

def run_simulation():
    session_id = str(uuid.uuid4())
    print(f"🚀 Starting Simulation [Session: {session_id}]")
    print(f"🌐 Testing Production URL: {BASE_URL}")
    print("---------------------------------------------------")
    ...
    try:
        resp = requests.post(f"{BASE_URL}/api/honey-pot", json=payload_1, headers=headers, timeout=30)
        data = resp.json()
    ...
    try:
        resp = requests.post(f"{BASE_URL}/api/honey-pot", json=payload_2, headers=headers, timeout=30)
        data = resp.json()
        agent_reply = data.get("agentReply", "")
        print(f"🤖 Agent: {agent_reply}")
    ...
    try:
        resp = requests.post(f"{BASE_URL}/api/honey-pot", json=payload_3, headers=headers, timeout=30)
        data = resp.json()
        print("\n✅ API Response (Intelligence Extracted):")
        print(json.dumps(data.get("extractedIntelligence"), indent=2))
        
        print("\n🎯 Callback should have been triggered on the server!")
        print(f"📊 Total Messages: {data.get('engagementMetrics', {}).get('totalMessagesExchanged', 'N/A')}")
    except Exception as e:
        print(f"❌ Error: {e}")
